...
- [ ] Full port of `impact_model_v4.1.py` (~15,700 lines) as the live Django scoring engine — Player Score (RMM), Compatibility Score (CS), Financial Fit (TFM), Transfer Probability. Phase 3 completed the prerequisite risk-mitigation work: all 20 duplicated function names catalogued with authoritative versions identified (2 escalated for direct human review), a full-population correctness-oracle snapshot (41,708 real players) versioned for Phase 5's parity testing, and a master curation map. Phase 3 also corrected a mis-assumption carried since early research — the script's only sklearn model (`RandomForestRegressor`) predicts transfer fee/value, not a transfer probability; it is now correctly mapped to Financial Fit (TFM) and versioned as a joblib artifact, while Transfer Probability is confirmed a fully deterministic weighted formula needing no ML. Phase 4 ports against this verified ground truth.
...
---
*Last updated: 2026-07-22 after Phase 3 (Scoring Engine Curation & Correctness Oracle) completion*
